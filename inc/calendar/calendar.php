...
class DHTML_Calendar {
    ...
    function __construct($calendar_lib_path = '/calendar/',
        ...
        $this->calendar_options = array('dateFormat' => $GLOBALS["DATETIME"]["dmY"]);
    ...
    function get_load_files_code() {
        $code  = ( '<link rel="stylesheet" type="text/css" media="all" href="' .
                   $this->calendar_lib_path.$this->calendar_theme_file.
									 '?'.filemtime($this->calendar_lib_path.$this->calendar_theme_file).
                   '" />' . NEWLINE );
        $code .= ( '<script type="text/javascript" src="' .
                   $this->calendar_lib_path . $this->calendar_file .
									 '?'.filemtime($this->calendar_lib_path.$this->calendar_file).
                   '"></script>' . NEWLINE );
        $code .= ( '<script type="text/javascript" src="' .
                   $this->calendar_lib_path . $this->calendar_lang_file .
									 '?'.filemtime($this->calendar_lib_path.$this->calendar_lang_file).
                   '"></script>' . NEWLINE );
        $code .= ( '<script type="text/javascript" src="' .
                   $this->calendar_lib_path . $this->calendar_setup_file .
									 '?'.filemtime($this->calendar_lib_path.$this->calendar_setup_file).
                   '"></script>' );
        return $code;
    }
    ...
    function make_input_field($cal_options = array(), $field_attributes = array()) {
        ...
        $return .= '<a href="#" id="'. $this->_trigger_id($id) . '">' .
            '<img align="middle" border="0" class="jsdate-image" src="' . $this->img_path . 'calendar.gif" alt="" /></a>';
