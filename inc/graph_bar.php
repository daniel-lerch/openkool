...
/**
 * Draws a bar chart
 * Pass values as GET parameters in the src argument for an img tag
 * e.g.: <img src="$ko_path/inc/graph_bar.php?data=1*2*3&label=one*two*three&title=Demo" />
 *
 * Possible arguments
 * - data: Values to be drawn separated by *
 * - label: Labels for the above data separated by *
 * - title: Title for bar chart
 * - title_x: Title text for x axes
 * - title_y: Title text for y axes
 * - size: Dimension of image as WxH, e.g. 300x150
 * - Options for the bar chart: yValueMode, xStep
 **/
...
//Prepare data array for bar chart
$bardata = array();
foreach($data as $k => $v) {
	$bardata[$label[$k]] = $v;
}
...
//Set chart size
$sizes = $_GET["size"];
if(!$sizes || FALSE === strpos($sizes, "x")) $sizes = "300x150";
list($size_x, $size_y) = explode("x", $sizes);


//Create bar chart
$bar = new PostGraph($size_x, $size_y);

//Set options from GET
if(isset($_GET["yValueMode"])) $bar->setOption("yValueMode", $_GET["yValueMode"]);
if(isset($_GET["xStep"])) $bar->setOption("xStep", $_GET["xStep"]);
if(isset($_GET["textXOrientation"])) $bar->setOption("textXOrientation", $_GET["textXOrientation"]);
if(isset($_GET['colorBars'])) $bar->setOption('colorBarsArray', array(hexdec(substr($_GET['colorBars'], 0, 2)), hexdec(substr($_GET['colorBars'], 2, 2)), hexdec(substr($_GET['colorBars'], 4, 2))));

//Add titles
$bar->setGraphTitles($_GET["title"], $_GET["title_x"], $_GET["title_y"]);

//Assign data and draw graph
$bar->setData($bardata);
$bar->drawImage();
$bar->printImage();
...
class PostGraph
{
    ...
    var $textXOrientation; // Text orientation at x axe. Usually normal.
		var $xStep; // Set this to 10 to only print each 10th tick on the x axis (or any other value of course)
    ...
    */
    function drawXAxe()
    {
        ...
        foreach($this->data as $key => $value) {
					//Only draw each xStep label if set
					if($this->xStep && $key % $this->xStep != 0) $key = "";
