...
function ko_list_trackings() {
	...
	if ($access['tracking']['MAX'] > 3) $list->setActionNew('new_tracking');
	$list->setSort(TRUE, 'setsort', $_SESSION['sort_trackings'], $_SESSION['sort_trackings_order']);
	$list->setStats($rows);
	$list->setColumnLink('name', 'index.php?action=select_tracking&amp;id=ID');
	$list->setRowClass("ko_list_hidden", 'return HIDDEN == 1;');
	$list->showColItemlist();
	$list->setWarning(kota_filter_get_warntext('ko_tracking'));
	//Output the list
	$list->render($es);
}//ko_list_trackings()
...
/**
 * This function is called after global ajax actions (defined in KOTA)
 * Depending on the current SESSION[show] it will display list of mod entries or normal ones
 * Necessary as both list show entries from same DB table and KOTA allows only one redraw function per DB table
 */
function ko_tracking_list_wrapper() {
	if($_SESSION['show'] == 'list_tracking_entries') {
		ko_list_tracking_entries();
	} else if($_SESSION['show'] == 'mod_entries') {
...
/**
 * List tracking entries as normal list view instead of form table
 */
function ko_list_tracking_entries() {
	global $smarty, $access;

	if($access['tracking']['MAX'] < 1) return;

	if(!$_SESSION['tracking_id'] || $access['tracking'][$_SESSION['tracking_id']] < 1) {
		print 'Select Tracking';
		return;
	}
	$tracking = db_select_data('ko_tracking', "WHERE `id` = '".$_SESSION['tracking_id']."'", '*', '', '', TRUE);

	apply_tracking_dates_filter($d_where, $z_limit, 'ko_tracking_entries');
	$z_where = " AND `tid` = '".intval($_SESSION['tracking_id'])."' ".$d_where;

	$kota_where = kota_apply_filter('ko_tracking_entries');
	if($kota_where != '') $z_where .= " AND ($kota_where) ";


	$order = 'ORDER BY '.$_SESSION['sort_tracking_entries'].' '.$_SESSION['sort_tracking_entries_order'];

	$rows = db_get_count('ko_tracking_entries', 'id', $z_where." AND `status` = '0'");
	if($_SESSION['show_start'] > $rows) {
		$_SESSION['show_start'] = 1;
		$z_limit = 'LIMIT '.($_SESSION['show_start']-1).', '.$_SESSION['show_limit'];
	}
	...
	$subtitle = '<i class="fa fa-list" title="'.getLL('tracking_list_subtitle_list').'"></i>&nbsp;&nbsp;<a href="/tracking/?action=enter_tracking&id='.$_SESSION['tracking_id'].'"><i class="fa fa-table" title="'.getLL('tracking_list_subtitle_table').'"></i></a>';
	$list->setSubTitle($subtitle);
	if($access['tracking'][$_SESSION['tracking_id']] > 1) $list->setActionNew('new_tracking_entry');
	$list->setAccessRights(array('edit' => 2, 'delete' => 2), $access['tracking']);
	$list->setActions(array(
		'edit' => array('action' => 'edit_tracking_entry'),
		'delete' => array('action' => 'delete_tracking_entry', 'confirm' => TRUE),
	));
	$list->setSort(TRUE, 'setsortentries', $_SESSION['sort_tracking_entries'], $_SESSION['sort_tracking_entries_order']);
	$list->setStats($rows);
...
function ko_list_tracking_mod_entries() {
	global $smarty;
	global $access;
	...
	$manualAccess = array('edit' => array());
	foreach ($es as $k => $te) {
		$t = db_select_data('ko_tracking', "WHERE `id` = {$te['tid']}", '*', '', '', TRUE);
		if ($t['id'] != $te['tid'] || $t['mode'] != 'type') {
			$edit = FALSE;
		} else {
			$edit = TRUE;
		}
	...
	$list->setAccessRights(array('check' => 2, 'edit' => 2, 'delete' => 2), $access['tracking']);
	$list->setActions(array(
		'edit' => array('action' => 'edit_tracking_mod'),
		'check' => array('action' => 'confirm_tracking_entry'),
		'delete' => array('action' => 'delete_tracking_entry', 'confirm' => TRUE),
	));
	$list->setManualAccess('edit', $manualAccess['edit']);
...
function ko_formular_tracking_entry_mod($id) {
	global $KOTA;

	unset ($KOTA['ko_tracking_entries']['status']);

	$te = db_select_data('ko_tracking_entries', "WHERE `id` = {$id}", '*', '', '', TRUE);
	if ($te['id'] != $id) return FALSE;
	$tracking = db_select_data('ko_tracking', "WHERE `id` = {$te['tid']}", '*', '', '', TRUE);
	if ($tracking['id'] != $te['tid']) return FALSE;
	if ($tracking['mode'] != 'type') return FALSE;
	...
	$KOTA['ko_tracking_entries']['type']['form']['values'] = $values;
	$KOTA['ko_tracking_entries']['type']['form']['descs'] = $values;

	$KOTA['ko_tracking_entries']['tid']['form']['type'] = 'html';
	//$KOTA['ko_tracking_entries']['tid']['form']['dontsave'] = TRUE;
	$KOTA['ko_tracking_entries']['tid']['form']['ignore_test'] = TRUE;
	$KOTA['ko_tracking_entries']['tid']['pre'] = 'db_get_column("ko_tracking", "@VALUE@", "name")';

	$dates = array();
	$peopleIds = ko_tracking_get_people($tracking['filter'], $dates, $tracking['id']);
	if (sizeof($peopleIds) > 0) {
		$KOTA['ko_tracking_entries']['lid']['form']['exclude_sql'] = "(`id` IN (".implode(',', array_map(function($el){return $el['id'];}, $peopleIds))."))";
	} else {
		$KOTA['ko_tracking_entries']['lid']['form']['exclude_sql'] = "(0=1)";
	}

	$form_data['title'] =  getLL('tracking_form_title_edit_mod');
	$form_data['submit_value'] = getLL('save');
	$form_data['action'] = 'submit_edit_tracking_mod';

	$form_data['action_as_new'] = 'submit_edit_approve_tracking_mod';
	$form_data['label_as_new'] = getLL('tracking_form_submit_edit_approve_mod');
	$form_data['cancel'] = 'list_trackings';

	ko_multiedit_formular('ko_tracking_entries', '', $id, '', $form_data);
}




function ko_formular_tracking_entry($mode, $id='') {
	global $KOTA, $access;

	if($mode == 'new') {
		$id = 0;
		if($_SESSION['tracking_id']) {
			//Set default values for tracking_id and date
			kota_assign_values('ko_tracking_entries', array('tid' => $_SESSION['tracking_id'], 'date' => date('d.m.Y')));
			//Set available types for the selected tracking
			$tracking = db_select_data('ko_tracking', "WHERE `id` = '".$_SESSION['tracking_id']."'", '*', '', '', TRUE);
		}
	} else if($mode == 'edit') {
		if(!$id) return FALSE;
		...
		$KOTA['ko_tracking_entries']['type']['form']['value'] = $te['type'];
	} else {
		return FALSE;
	}

	//Set available types from tracking (for edit and new)
	$values = array('');
	foreach (explode("\n", $tracking['types']) as $type) {
		$type = trim($type);
		if (!$type) continue;
	...
	$form_data['title'] =  $mode == 'new' ? getLL('tracking_entry_form_title_new') : getLL('tracking_entry_form_title_edit');
	$form_data['submit_value'] = getLL('save');
	$form_data['action'] = $mode == 'new' ? 'submit_new_tracking_entry' : 'submit_edit_tracking_entry';
	if($mode == 'edit' && ($access['tracking']['ALL'] > 1 || $access['tracking'][$id] > 1)) {
		$form_data['action_as_new'] = 'submit_as_new_tracking_entry';
		$form_data['label_as_new'] = getLL('tracking_entry_form_submit_as_new');
	}
	$form_data['cancel'] = 'list_tracking_entries';

	ko_multiedit_formular('ko_tracking_entries', '', $id, '', $form_data);
}//ko_formular_tracking_entry()
 ...
 */
function ko_tracking_enter_form() {
			...
			foreach($columns as $col) {
				...
				if(!isset($testp[$col])) {
					foreach($people as $pid => $p) {
						$people[$pid][$col] = strip_tags(ko_unhtml(map_leute_daten($p[$col], $col, $p, $all_datafields)));
					}
				}
				$show_cols[] = $col;
	...
	if($tracking['mode'] == 'simple' || $tracking['mode'] == 'valueNonNum') {
		...
		foreach ($sum as $date => $s) {
			$sum[$date] = '<a href="/leute?action=set_trackingentries_filter&tid='.$tracking['id'].'&from='.$date.'&to='.$date.'&value=1">'.$s.'</a>';
		}
		...
		foreach($sums as $date => $values) {
			$_sum = array();
			foreach($types as $t) {
				if($values[$t['value']]) $_sum[] = '<a href="/leute?action=set_trackingentries_filter&tid='.$tracking['id'].'&from='.$date.'&to='.$date.'&value='.$t['value'].'"">'.$values[$t['value']].'x'.$t['value'].'</a>';
			}
	...
	$subtitle = '<a href="/tracking/?action=list_tracking_entries&id='.$_SESSION['tracking_id'].'"><i class="fa fa-list" title="'.getLL('tracking_list_subtitle_list').'"></i></a>&nbsp;&nbsp;<i class="fa fa-table" title="'.getLL('tracking_list_subtitle_table').'"></i>';
	...
	$smarty->assign('tracking_order_people', ko_get_userpref($_SESSION['ses_userid'], 'tracking_order_people'));
 ...
 * @param string $_start: If set this date will be used as first date, otherwise _SESSION[date_start] will be used (date)
 * @param int limit: Set a maximum number of dates to return. If not set, _SESSION[date_limit] will be used
 * @param string &$prev: Value for backwards date navigation (date)
 * @param string &$next: Value for forward date navigation (date)
 * @param string &$prev1: Value to navigation back one day (date)
...
function ko_tracking_get_dates($tracking, $_start='', $_limit='', &$prev, &$next, &$prev1, $apply_filter=TRUE) {
	...
	$dates = array();
	$_dates = $neg_dates = $_dates_subtitles = array();
			...
			foreach($eg_dates as $d) {
				if($neg) {
					...
					if($d['startdatum'] != $d['enddatum']) {
						...
						while($cd != $d['enddatum']) {
							$_dates[] = $cd;
							$_dates_subtitles[$cd] = $d['title'];
							$cd = add2date($cd, 'day', 1, TRUE);
						}
						$_dates[] = $d['enddatum'];
						$_dates_subtitles[$d['enddatum']] = $d['title'];
					} else {
						$_dates[] = $d['startdatum'];
						$_dates_subtitles[$d['startdatum']] = $d['title'];
					}
				}
			}
		}
	...
	//Remove double dates (may come from eventgroups)
	$checked = array();
	foreach($_dates as $k => $v) {
		...
		if((int)str_replace('-', '', $date) >= (int)str_replace('-', '', $start)) {
			if($num_dates < $limit) {
				$dates[] = array('date' => $date,
												 'title' => strftime($DATETIME[$dateformat], strtotime($date)),
												 'subtitle' => $_dates_subtitles[$date],
...
function ko_tracking_get_people($filters, &$dates, $tid, $apply_filters=FALSE) {
	...
	if (isset($_SESSION['tracking_filter']['peoplesearch'])) {
		$search = format_userinput($_SESSION['tracking_filter']['peoplesearch'], 'text');

		$tokens = explode(' ', $search);
		$fields = array('vorname', 'nachname', 'firm');
...
function ko_tracking_settings() {
	...
	$rowcounter++;
	$value = ko_get_userpref($_SESSION['ses_userid'], 'tracking_default_show');
	$values = $descs = array();
	$frmgroup[$gc]['row'][$rowcounter]['inputs'][0] = array('desc' => getLL('tracking_settings_default_show'),
			'type' => 'select',
			'name' => 'sel_default_show',
			'values' => array('enter_tracking', 'list_tracking_entries'),
			'descs' => array(getLL('submenu_tracking_enter_tracking'), getLL('submenu_tracking_list_tracking_entries')),
			'value' => $value,
	);
	$rowcounter++;
	...
	if($access['tracking']['MAX'] > 3) {
		...
		if ($_SESSION['ses_userid'] == ko_get_root_id()) {
			$value = ko_get_setting('tracking_enable_checkin');
			$frmgroup[$gc]['row'][$rowcounter++]['inputs'][0] = array('desc' => getLL('tracking_settings_enable_checkin'),
				'type' => 'switch',
				'name' => 'chk_enable_checkin',
				'value' => $value,
			);
		}

		if ($_SESSION['ses_userid'] == ko_get_root_id() || ko_get_setting('tracking_enable_checkin')) {
			$values = array('vorname', 'nachname', 'adresse', 'plz', 'ort', 'geburtsdatum');
			$descs = ko_array_ll($values, 'kota_ko_leute_');
			$avalue = ko_get_setting('checkin_display_leute_fields');
			$avalues = $adescs = array();
			foreach (explode(',', $avalue) as $av) {
				$av = trim($av);
				if (!$av) continue;

				if (($k = array_search($av, $values)) !== FALSE) {
					$avalues[] = $av;
					$adescs[] = $descs[$k];
				}
			}
			$frmgroup[$gc]['row'][$rowcounter]['inputs'][0] = array('desc' => getLL('tracking_settings_checkin_display_leute_fields'),
				'type' => 'checkboxes',
				'name' => 'sel_checkin_display_leute_fields',
				'avalue' => $avalue,
				'avalues' => $avalues,
				'adescs' => $adescs,
				'values' => $values,
			...
			$value = ko_get_setting('checkin_max_results');
			$frmgroup[$gc]['row'][$rowcounter++]['inputs'][1] = array('desc' => getLL('tracking_settings_checkin_max_results'),
				'type' => 'text',
				'name' => 'txt_checkin_max_results',
				'value' => $value,
			);
		}
	}
	...
	hook_form('tracking_settings', $frmgroup, '', '');
 ...
 * @param int addsums: 0 (default): Don't show sums, 1: Add sums, 2: Only show sums
 * @return int error: If there is an error during the LaTeX conversion to PDF an error code will be returned
 */
function ko_tracking_export($mode, $filename, $id, $address_columns='name', $dates='current', $layout='L', $addrows=0, $combine_families=FALSE, $addsums=0) {
	...
	switch($dates) {
		case 'current':
			$dates = ko_tracking_get_dates($tracking, '', '', $prev, $next, $prev1);
		break;
		case 'filter':
			if($_SESSION['tracking_filter']['date1'] && $_SESSION['tracking_filter']['date2']) {
				$dates = ko_tracking_get_dates($tracking, $_SESSION['tracking_filter']['date1'], 10000, $prev, $next, $prev1);
			} else if($_SESSION['tracking_filter']['date1']) {
				$dates = ko_tracking_get_dates($tracking, $_SESSION['tracking_filter']['date1'], 250, $prev, $next, $prev1);
			} else if($_SESSION['tracking_filter']['date2']) {
				$dates = ko_tracking_get_dates($tracking, -1, 250, $prev, $next, $prev1);
			} else {
				$dates = ko_tracking_get_dates($tracking, '', '', $prev, $next, $prev1);
			}
		break;
	...
	if($tracking['mode'] != 'type' && $tracking['mode'] != 'typecheck') $where .= " AND `type` = '' ";
	$rows = db_select_data('ko_tracking_entries', $where, '*', 'ORDER BY lid,date ASC');
	foreach($rows as $row) {
		if($tracking['mode'] == 'type' || $tracking['mode'] == 'typecheck') {
	...
	if($combine_families) {
		...
		foreach($people as $pid => $p) {
			...
			if(!$p['famid']) {
				foreach($columns as $col) {
					$data[$row][] = strip_tags(ko_unhtml(map_leute_daten($p[$col], $col, $p, $all_datafields)));
				}
				$sums = array();
				$famSums = array();
				foreach($dates as $date) {
					if($tracking['mode'] == 'simple') {
					...
					} else if($tracking['mode'] == 'type' || $tracking['mode'] == 'typecheck') {
						$value = '';
						...
						if(function_exists('my_'.$tracking['mode'].'_psum')) {
							$oneNum = call_user_func('my_'.$tracking['mode'].'_psum', array('value' => $entries[$p['id']][$date['date']]));
							$sums['v'] += $oneNum;
							$value = $oneNum > 0 ? $oneNum : '';
						}
					}

					if($addsums != 2) $data[$row][] = trim($value);
				}
				//if($mode == 'xls') {
					if($tracking['mode'] == 'type' || $tracking['mode'] == 'typecheck') {
						foreach ($famSumCols as $fsc) {
							if($addsums != 2) $data[$row][] = ($famSums[$fsc] == '' || !isset($famSums[$fsc])) ? 0 : $famSums[$fsc];
						}
					}
				//}
				//Add sum column
					...
					case 'vorname':
						if(!in_array('anrede', $columns)) {
							...
							if($fam['famfirstname']) {
								...
								$parents = db_select_data('ko_leute', "WHERE `famid` = '".$p['famid']."' AND `famfunction` IN ('husband', 'wife') AND `deleted` = '0'".ko_get_leute_hidden_sql(), 'famfunction,vorname', 'ORDER BY famfunction ASC');
								$parent_values = array();
				...
				switch($tracking['mode']) {
					...
					default:
						if(substr($tracking['mode'], 0, 8) == 'bitmask_') {
							$oneValue = 0;
							foreach($famdata[$p['famid']][$date['date']] as $d) {
								$oneNum = call_user_func('my_'.$tracking['mode'].'_psum', array('value' => $d['value']));
								if($oneNum > 0)  {
									$sums['p'] += $oneNum;
									$oneValue += $oneNum;
								}
							}
							$value[] = $oneValue > 0 ? $oneValue : '';
						}

				}
				if($addsums != 2) $data[$row][] = implode(', ', $value);
			}
			...
			} else if($tracking['mode'] == 'type' || $tracking['mode'] == 'typecheck') {  //Don't include p/c for type sums
		...
		foreach($people as $p) {
			foreach($columns as $col) {
				$v = strip_tags(ko_unhtml(map_leute_daten($p[$col], $col, $p, $all_datafields)));
	...
	if($mode == 'xls') {
		...
		if($addsums > 0) {
			...
			$col_headers[] = getLL('tracking_list_total');
		}
		...
		if(mb_substr($first_filter, 0, 1) == 'g' && mb_strlen($first_filter) >= 7) {
			...
			$subtitles[getLL('groups_groups')] = $group['name'];
	...
	//Create PDF file
		...
		if(substr($tracking['mode'], 0, 7) == 'bitmask' || $tracking['mode'] == 'type' || $tracking['mode'] == 'typecheck') {
			$data_width_min = 35;
			$data_width_max = 50;
		}
		else {
		...
		$sum_width = 25;
		$num_sum_cols = 1;
		if($addsums > 0 && ($tracking['mode'] == 'type' || $tracking['mode'] == 'typecheck')) $num_sum_cols += sizeof($famSumCols);
		...
		if ($addsums > 0) {
			if ($num_dates >= $cols_fitting) {
				...
				if ($cols_fitting_with_sum < $num_dates) {
					...
					$data_width = ((float)($pdf->layout['page_width_usable'] - $num_cols * $col_width - $num_sum_cols*$sum_width) / (float)$num_dates);
					if ($data_width > $data_width_max) {
						$data_width = $data_width_max;
					}
				}
		...
		for($t=1; $t<=$tables; $t++) {
			...
			foreach($data as $key => $row) {
				...
				if($t == $tables && $addsums > 0) {
					$sumCounter = 0;
					foreach($famSumCols as $sumCol) {
						$ldata[$key][] = $row[$num_cols+$stop+$sumCounter];
						$sumCounter++;
					}
					$ldata[$key][] = $row[$num_cols+$stop+$sumCounter];
				}
 ...
 * @param string $filter: groupID, smallgroupID or serialized filter preset
 * @returns string Name of the group, smallgroup of filter preset
 */
function ko_tracking_get_filter_name($filter, $returnTitle=FALSE) {
	//Group id
	if(substr($filter, 0, 1) == 'g' && strlen($filter) >= 7) {
		$name = $title = ko_groups_decode($filter, 'group_desc_full');
		if ($returnTitle) {
			$all = ko_groups_decode($filter, 'full_gid');
			$title = ko_groups_decode($all, 'group_desc_full');
	...
	//Small group id
		...
		if ($returnTitle) return array($sg['name'], $sg['name']);
		else return $sg['name'];
	}
	//Filter preset
	else if(mb_substr($filter, 0, 1) == 'F') {
		...
		foreach($filterset as $f) {
			if($f['value'] == base64_decode(mb_substr($filter, 1))) {
				if ($returnTitle) return array($f['key'], $f['key']);
				else return $f['key'];
			}
		}
	}
